...
if (!app.includes('function AIEducationGuidance(') || !app.includes('function MajorMatches(') || !app.includes('assessment-ai-orbit-mark') || !styles.includes('.assessment-ai-orbit-mark') || !styles.includes('.education-ai-guidance') || !app.includes('strongest_major') || !app.includes('minor_guidance')) throw new Error('Assessment-based major and minor guidance or the Naseeb logo mark is missing.');
...
if (!app.includes('function ReasoningRunner(') || app.includes('YOUR SNAPSHOT') || app.includes('ESTIMATED COGNITIVE SCORE') || !app.includes('assessment-number-breakdown') || !app.includes('assessment-score-fold') || !app.includes('function AssessmentResultPage(') || !challenges.includes("instrument: 'ICAR-16'") || !challenges.includes('estimatedCognitiveScore') || !challenges.includes("sourceId: 'MR.45'") || !challenges.includes("sourceId: 'R3D.08'") || !styles.includes('.reasoning-options') || !styles.includes('.reasoning-item-image') || !styles.includes('.assessment-number-column')) throw new Error('The ICAR-16 challenge, result pages, or expandable score breakdown is missing, or the retired snapshot remains.');
