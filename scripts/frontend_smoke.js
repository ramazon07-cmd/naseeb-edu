...
if (!app.includes('const personalityDone = CHALLENGES.filter') || !app.includes('attempts={data.challengeAttempts}') || !app.includes('function DashboardUniversityCard(') || !app.includes("setPage('college_search')") || !styles.includes('.dashboard-discovery-card')) throw new Error('The student dashboard must report personality progress as a live-progress row and keep the University Match card.');
