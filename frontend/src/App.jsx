  ...
  return <div className="app-shell">
    <aside className={`sidebar ${mobileOpen ? 'open' : ''}`}>
      ...
      <nav>{navigation.map((item) => {
          ...
          return <button key={item} className={page === item ? "active" : ''} onClick={() => {setPage(item);setQuery('');setSearchOpen(false);setMobileOpen(false);}}><ItemIcon size={18} /><span>{t(PAGE_META[item].label)}</span>{item === 'support' && supportBadge > 0 && <span className="nav-badge">{supportBadge > 99 ? '99+' : supportBadge}</span>}</button>;
        })}</nav>
      <div className="sidebar-profile"><span className="avatar">{initials(fullName(user))}</span><div><b>{fullName(user)}</b><small>{label(user.role)}</small></div><button className="icon-button" onClick={logout} title={t('Logout')}><LogOut size={18} /></button></div>
    </aside>
...
function StudentOverview({ student, data, onBack, user, notify }) {
  ...
  return <div className="section-stack student-overview">
    <section className="student-overview-hero">
      <div className="student-overview-title">{onBack && <button className="button quiet" onClick={onBack}>{t("← Students")}</button>}<div className="profile-identity"><span className="avatar large">{initials(fullName(student.user_detail))}</span><div><span className="eyebrow">{t("STUDENT 360° PROFILE")}</span><h2>{fullName(student.user_detail)}</h2><p>{student.user_detail?.email} • {student.school_name || t("No school assigned")}</p></div></div><div className="panel-actions">{['admin', 'counselor', 'organization'].includes(user?.role) && <button className="button quiet" onClick={() => setCredentialOpen(true)}><Fingerprint size={16} /> {t('Reset login')}</button>}{['admin', 'counselor'].includes(user?.role) && <button className="button quiet" onClick={() => setParentInviteOpen(true)}><UsersRound size={16} /> {t("Invite parent")}</button>}</div></div>
...
function StudentForm({ user, data, student, onClose, onSaved, notify }) {
  ...
  return <Modal title={student ? t("Edit student") : t("Add student")} onClose={onClose}><form className="form-grid" onSubmit={submit}>
    ...
    <Field label={t("Notes")}><textarea value={form.notes} onChange={(e) => update('notes', e.target.value)} /></Field>
...
function DocumentForm({ user, data, defaultType = '', onClose, onSaved, notify }) {
  ...
  async function submit(event) {
    ...
    if (isCounselor(user)) payload.append('counselor_comment', values.get('counselor_comment') || '');
