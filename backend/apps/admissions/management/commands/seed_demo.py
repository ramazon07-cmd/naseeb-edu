...
class Command(BaseCommand):
    ...
    # Regional demo coverage. Gives /api/public/reach/ non-trivial development
    # data without touching the primary demo workspace. Guarded by the same
    # DEMO_ACCOUNTS_ENABLED check as the rest of this command (see handle()).
    REGIONAL_COVERAGE = (
    ...
    def seed_regional_coverage(self):
        """Create demo schools across several regions with count-only students.

        These accounts exist purely so the public reach aggregate has realistic
        development data; they are never handed out as logins, so they get an
        unusable password instead of a demo password.
        """
